...
impl NuomiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("糯米", "🍓糯米┃秒播", "https://www.nuomi.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "糯米", items.len()),
                        title,
...
impl Default for NuomiScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for NuomiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = NuomiScraper::new();
        test_scraper(&scraper, "糯米", TEST_KEYWORD).await
            .expect("糯米 test failed");
    }
}
