...
impl YisoScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("易搜", "😹易搜┃阿狸", "https://www.yiso.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "易搜", items.len()),
                        title,
...
impl Default for YisoScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for YisoScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = YisoScraper::new();
        test_scraper(&scraper, "易搜", TEST_KEYWORD).await
            .expect("易搜 test failed");
    }
}
