...
impl BaibaiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("白白", "🐟白白┃秒播", "https://www.baibai.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "白白", items.len()),
                        title,
...
impl Default for BaibaiScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for BaibaiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = BaibaiScraper::new();
        test_scraper(&scraper, "白白", TEST_KEYWORD).await
            .expect("白白 test failed");
    }
}
