...
impl BiliScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Bili", "🅱哔哔合集┃弹幕", "https://www.bili.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Bili", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Bili", ids),
...
impl Default for BiliScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for BiliScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = BiliScraper::new();
        test_scraper(&scraper, "Bili", TEST_KEYWORD).await
            .expect("Bili test failed");
    }
}
