...
impl CcScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("cc", "请勿相信视频中广告", "https://www.cc.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "cc", items.len()),
                        title,
...
impl Default for CcScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for CcScraper {
