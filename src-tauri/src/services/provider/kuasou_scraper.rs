...
impl KuasouScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("夸搜", "😻夸搜┃夸父", "https://www.kuasou.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "夸搜", items.len()),
                        title,
...
impl Default for KuasouScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for KuasouScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = KuasouScraper::new();
        test_scraper(&scraper, "夸搜", TEST_KEYWORD).await
            .expect("夸搜 test failed");
    }
}
