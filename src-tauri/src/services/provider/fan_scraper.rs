...
impl FanScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("fan", "导航www.饭太硬.com", "https://www.fan.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "fan", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "fan", ids),
...
impl Default for FanScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for FanScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = FanScraper::new();
        test_scraper(&scraper, "fan", TEST_KEYWORD).await
            .expect("fan test failed");
    }
}
