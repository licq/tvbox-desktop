...
impl BiliychScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Biliych", "🅱哔哔演唱会┃弹幕", "https://www.biliych.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Biliych", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Biliych", ids),
...
impl Default for BiliychScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for BiliychScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = BiliychScraper::new();
        test_scraper(&scraper, "Biliych", TEST_KEYWORD).await
            .expect("Biliych test failed");
    }
}
