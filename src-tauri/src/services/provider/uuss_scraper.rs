...
impl UussScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("UC", "🌈优汐┃搜搜", "https://www.ucss.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "UC", items.len()),
                        title,
...
impl Default for UussScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for UussScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = UussScraper::new();
        test_scraper(&scraper, "UC", TEST_KEYWORD).await
            .expect("UC test failed");
    }
}
