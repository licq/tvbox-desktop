...
/// wencai native scraper for https://www.wencai.com/
/// Uses csp_JpysGuard for playback resolution.
pub struct WencaiScraper {
    base: NativeScraper,
}
...
impl WencaiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("wencai", "💮文采┃秒播", "https://www.wencai.com"),
        }
    }
    ...
    pub async fn detail(&self, ids: &str) -> Result<Option<ScrapedCatalogItem>, ProviderError> {
        // Try common detail URL patterns for wencai
    ...
    fn parse_search_results(&self, body: &str) -> Result<Vec<ScrapedCatalogItem>, ProviderError> {
        ...
        // Look for common patterns in wencai search results
        // Typically vod detail links contain /vod/ or /play/ paths
        for line in body.lines() {
            let line = line.trim();
            if line.contains("/vod/") || line.contains("/play/") {
                if let Some(title) = self.extract_title_from_line(line) {
                    let source_item_key = format!("wencai-{}", items.len());
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Result<Option<ScrapedCatalogItem>, ProviderError> {
        ...
        // Look for episode/play links in detail page
        for line in body.lines() {
            let line = line.trim();
        ...
        Ok(Some(ScrapedCatalogItem {
            source_item_key: format!("wencai-{}", ids),
    ...
    fn extract_play_url(&self, line: &str) -> String {
        if let Some(href_start) = line.find("href=\"") {
            ...
            if let Some(href_end) = remaining.find('"') {
                ...
                if url.starts_with('/') {
                    return format!("https://www.wencai.com{}", url);
                }
...
impl Default for WencaiScraper {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl VideoProvider for WencaiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = WencaiScraper::new();
        test_scraper(&scraper, "wencai", TEST_KEYWORD).await
            .expect("wencai test failed");
    }
}
