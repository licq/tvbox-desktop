...
impl YcyzScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("原创", "☀原创┃不卡", "https://www.yuanchuang.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "原创", items.len()),
                        title,
...
impl Default for YcyzScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for YcyzScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = YcyzScraper::new();
        test_scraper(&scraper, "原创", TEST_KEYWORD).await
            .expect("原创 test failed");
    }
}
