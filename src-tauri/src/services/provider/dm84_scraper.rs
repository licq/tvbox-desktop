...
impl Dm84Scraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Dm84", "🚌巴士┃动漫", "https://www.dm84.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Dm84", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Dm84", ids),
...
impl Default for Dm84Scraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for Dm84Scraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = Dm84Scraper::new();
        test_scraper(&scraper, "Dm84", TEST_KEYWORD).await
            .expect("Dm84 test failed");
    }
}
