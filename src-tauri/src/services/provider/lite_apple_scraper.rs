...
pub struct LiteAppleScraper {
    base: NativeScraper,
}

impl LiteAppleScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("苹果", "🍎苹果┃不卡", "https://www.pingguo.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "苹果", items.len()),
                        title,
...
impl Default for LiteAppleScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for LiteAppleScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = LiteAppleScraper::new();
        test_scraper(&scraper, "苹果", TEST_KEYWORD).await
            .expect("苹果 test failed");
    }
}
