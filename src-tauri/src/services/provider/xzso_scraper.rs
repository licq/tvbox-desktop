...
impl XzsoScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("xzso", "👻盘它┃三盘", "https://www.xzso.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "xzso", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "xzso", ids),
...
impl Default for XzsoScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for XzsoScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = XzsoScraper::new();
        test_scraper(&scraper, "xzso", TEST_KEYWORD).await
            .expect("xzso test failed");
    }
}
