...
impl KkssScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("抠搜", "🍄抠抠┃搜搜", "https://www.kkss.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "抠搜", items.len()),
                        title,
...
impl Default for KkssScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for KkssScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = KkssScraper::new();
        test_scraper(&scraper, "抠搜", TEST_KEYWORD).await
            .expect("抠搜 test failed");
    }
}
