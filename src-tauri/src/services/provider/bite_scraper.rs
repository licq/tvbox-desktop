...
impl BiteScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("比特", "🍄比特┃手机", "https://www.bite.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "比特", items.len()),
                        title,
...
impl Default for BiteScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for BiteScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = BiteScraper::new();
        test_scraper(&scraper, "比特", TEST_KEYWORD).await
            .expect("比特 test failed");
    }
}
