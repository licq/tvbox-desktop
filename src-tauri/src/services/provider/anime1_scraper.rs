...
impl Anime1Scraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Anime1", "🐾日本┃动漫", "https://www.anime1.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Anime1", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Anime1", ids),
...
impl Default for Anime1Scraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for Anime1Scraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = Anime1Scraper::new();
        test_scraper(&scraper, "Anime1", TEST_KEYWORD).await
            .expect("Anime1 test failed");
    }
}
