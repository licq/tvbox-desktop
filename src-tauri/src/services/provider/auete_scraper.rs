...
/// auete native scraper for https://auete.com/
...
impl AueteScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("auete", "🏝奥特┃多线", "https://auete.com"),
        }
    }
    ...
    pub async fn play(&self, _flag: &str, play_url: &str) -> Result<Vec<PlaybackTarget>, ProviderError> {
        // auete play pages are resolved by resolver.rs
    ...
    fn parse_search_results(&self, body: &str) -> Result<Vec<ScrapedCatalogItem>, ProviderError> {
        ...
        // auete search results typically have links like /video/xxx or /vod/xxx
        for line in body.lines() {
            let line = line.trim();
            if line.contains("/video/") {
                if let Some(title) = self.extract_title_from_line(line) {
                    let source_item_key = format!("auete-{}", items.len());
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Result<Option<ScrapedCatalogItem>, ProviderError> {
        ...
        for line in body.lines() {
            let line = line.trim();
            if line.contains("/video/") || line.contains("/play/") {
    ...
    fn extract_play_url(&self, line: &str) -> String {
        ...
        if let Some(href_start) = line.find("href=\"") {
            ...
            if let Some(href_end) = remaining.find('"') {
                ...
                if url.starts_with('/') {
                    return format!("https://auete.com{}", url);
                }
