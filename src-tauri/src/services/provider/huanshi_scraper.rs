...
impl HuanshiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("欢视", "👓欢视┃多线", "https://www.huanshi.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "欢视", items.len()),
                        title,
...
impl Default for HuanshiScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for HuanshiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = HuanshiScraper::new();
        test_scraper(&scraper, "欢视", TEST_KEYWORD).await
            .expect("欢视 test failed");
    }
}
