...
impl DdrkScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("低端", "⏮️低端┃外剧", "https://www.ddrk.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "低端", items.len()),
                        title,
...
impl Default for DdrkScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for DdrkScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = DdrkScraper::new();
        test_scraper(&scraper, "低端", TEST_KEYWORD).await
            .expect("低端 test failed");
    }
}
