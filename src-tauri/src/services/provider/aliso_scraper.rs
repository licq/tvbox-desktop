...
impl AlisoScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Aliso", "🙀盘搜┃阿狸", "https://www.aliso.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Aliso", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Aliso", ids),
...
impl Default for AlisoScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for AlisoScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = AlisoScraper::new();
        test_scraper(&scraper, "Aliso", TEST_KEYWORD).await
            .expect("Aliso test failed");
    }
}
