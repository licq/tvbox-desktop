...
impl MisoScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("米搜", "🦋米搜┃夸父", "https://www.miso.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "米搜", items.len()),
                        title,
...
impl Default for MisoScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for MisoScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = MisoScraper::new();
        test_scraper(&scraper, "米搜", TEST_KEYWORD).await
            .expect("米搜 test failed");
    }
}
