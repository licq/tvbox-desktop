...
impl YsjScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("Ysj", "🎀异界┃动漫", "https://www.ysj.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "Ysj", items.len()),
                        title,
    ...
    fn parse_detail_page(&self, body: &str, ids: &str) -> Option<ScrapedCatalogItem> {
        ...
        Some(ScrapedCatalogItem {
            source_item_key: format!("{}-{}", "Ysj", ids),
...
impl Default for YsjScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for YsjScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = YsjScraper::new();
        test_scraper(&scraper, "Ysj", TEST_KEYWORD).await
            .expect("Ysj test failed");
    }
}
