...
impl MengmiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("萌米", "👀萌米┃多线", "https://www.mengmi.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "萌米", items.len()),
                        title,
...
impl Default for MengmiScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for MengmiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = MengmiScraper::new();
        test_scraper(&scraper, "萌米", TEST_KEYWORD).await
            .expect("萌米 test failed");
    }
}
