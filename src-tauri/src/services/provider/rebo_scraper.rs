...
impl ReboScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("热播", "📺热播┃多线", "https://www.rebo.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "热播", items.len()),
                        title,
...
impl Default for ReboScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for ReboScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = ReboScraper::new();
        test_scraper(&scraper, "热播", TEST_KEYWORD).await
            .expect("热播 test failed");
    }
}
