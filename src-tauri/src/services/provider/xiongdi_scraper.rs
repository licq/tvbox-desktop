...
impl XiongdiScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("兄弟", "🍊水星┃多线", "https://www.xingsheng.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "兄弟", items.len()),
                        title,
...
impl Default for XiongdiScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for XiongdiScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = XiongdiScraper::new();
        test_scraper(&scraper, "兄弟", TEST_KEYWORD).await
            .expect("兄弟 test failed");
    }
}
