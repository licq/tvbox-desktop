...
pub struct ChangzhangScraper {
    base: NativeScraper,
}

impl ChangzhangScraper {
    pub fn new() -> Self {
        Self {
            base: NativeScraper::new("厂长", "📔厂长┃不卡", "https://www.chz.com"),
        }
    }
            ...
            if line.contains("/vod/") || line.contains("/play/") || line.contains("/detail/") {
                if let Some(title) = self.extract_title(line) {
                    items.push(ScrapedCatalogItem {
                        source_item_key: format!("{}-{}", "厂长", items.len()),
                        title,
...
impl Default for ChangzhangScraper {
    fn default() -> Self { Self::new() }
}

#[async_trait]
impl VideoProvider for ChangzhangScraper {
...
mod tests {
    ...
    async fn test_search_then_detail_then_play() {
        let scraper = ChangzhangScraper::new();
        test_scraper(&scraper, "厂长", TEST_KEYWORD).await
            .expect("厂长 test failed");
    }
}
