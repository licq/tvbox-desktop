...
describe('playback ad blocking helpers', () => {
  it('classifies HLS manifests and direct segments', () => {
    ...
    expect(classifyPlaybackRequest('https://cdn.example.com/live/index.m3u8?token=abc#frag')).toBe('manifest')
  ...
  it('flags obvious ad resources but not player assets', () => {
    ...
    expect(isPlaybackAdResource('https://cdn.example.com/video.ts?asset=adservice#frag')).toBe(false)
